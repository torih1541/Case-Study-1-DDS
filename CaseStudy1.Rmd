...
breweries <- breweries %>%
  mutate(State = str_trim(State), # Trim leading and trailing spaces
         State = toupper(State), # Convert to uppercase to match the keys in state_map
...
states_map$region <- sapply(states_map$region, toTitleCase)

map_data <- merge(states_map, breweries_by_state, by.x = "region", by.y = "StateFullName", all.x = TRUE)

ggplot(data = map_data, aes(x = long, y = lat, group = group, fill = NumberOfBreweries)) +
  geom_polygon(color = "white") +
  scale_fill_gradient(low = "lightblue", high = "darkblue", name = "Brewery Count") +
  labs(title = "Number of Breweries by State", x = "", y = "") +
  theme_minimal() +
  theme(axis.text = element_blank(), axis.title = element_blank(), panel.grid = element_blank(), panel.background = element_blank())
